...
def home(request):
    ...
    if request.user.is_authenticated:
        ...
        playlists = list(Playlist.objects.filter(creator=request.user).exclude(name="Liked Songs"))
        ...
        liked_songs_playlist = Playlist.objects.filter(creator=request.user, name="Liked Songs").first()
...
def show_single_playlist(request, playlist_id):
    ...
    filtered_songs = all_songs.exclude(id__in=songs_in_playlist.values_list('id', flat=True))
    liked_song_ids = set(request.user.like_set.values_list('song_id', flat=True))
    ...
    if request.method == 'POST':
        ...
        return redirect('main:show_single_playlist', playlist_id=playlist_id)  # Redirect to refresh the page
